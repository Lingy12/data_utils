...
def process_file(args) -> Tuple[int, str, str, str, dict]:
    file, audio_check, sr = args
    file_path = file
    meta_path = file[:-5] + '.json'
    try:
        with open(meta_path, 'r') as f:
            meta_data = json.load(f)
    except:
        return -1, 'Fetching meta data failed for {}'.format(file_path), "", "", {}
...
def form_ds(librilght_raw:str, dest:str, sr=None, audio_check:bool=True, num_worker:int = 4):
    ...
    inputs = [(file, audio_check, sr) for file in files]
    ...
    with Pool(processes=num_worker) as pool:
        ...
        for res in results:
            ...
            if status == 1:
                ...
                # ds_dict["path"].append(str(file_path)) # no need to store local path
